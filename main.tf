...
resource "aws_s3_bucket" "request_bucket" {
  bucket = "saajman-translate-requests"
  force_destroy = true
}
 
# Response bucket (output JSON)
resource "aws_s3_bucket" "response_bucket" {
  bucket = "saajman-translate-responses"
  force_destroy = true
}
...
resource "aws_iam_role" "lambda_role" {
  name = "saajman-translate-lambda-role"
...
resource "aws_lambda_function" "translate_lambda" {
  function_name = "saajman-translate-lambda"
